...
- 同步状态：登录/验证为 `blocked`；页面总数大于 0 但结构化结果为 0 时为 `partial`；其余为 `completed`。所有状态均保存快照。
...
- 浏览器报告增加结构化视频数及播放/点赞/评论/分享汇总，并标记为当前列表最新快照口径。
...
| 风险 | 影响 | 对策 |
|---|---|---|
| CSS module 哈希变化 | 选择器失效 | 使用语义文本、结构边界和类名前缀组合；零解析时保留快照并返回 `partial` |
...
| 真实数据误入 Git | 隐私泄露 | 最终审查检查 `git status`，确保 `data/` 和 profile 未出现 |
...
- 回归测试：全部现有测试继续通过。
- 手动验证：真实页面加载数达到页面声明总数，重复同步后 `videos` 数量不增加，并能读取最新指标；确认 Git 不包含真实逐值数据。
